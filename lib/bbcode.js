...
exports.parse = function(post, cb) {
  ...
  var tagname_re = /^\/?(?:b|i|u|pre|center|samp|code|colou?r|size|noparse|url|link|s|q|(block)?quote|img|u?list|youtube|li)$/i;
  ...
  // boilerplate function to convert youtube urls to embeddable
  function youtubeUrlConvert(string)
  {
  ...
  function textToHtmlCB(mstr, m1, m2, m3, m4, offset, string)
  {
    ...
    if(isValidTag(m2)) {
      ...
      switch (m2l) {
        ...
        case "youtube":
          opentags.push(new taginfo_t('youtube', '</iframe>'));

          urlstart = mstr.length + offset;

          return '<iframe id="ytplayer" type="text/html" width="640" height="390" src="';
    ...
    if(isValidTag(m4)) {
      ...
      if(m4l == "url" || m4l == "link" || "youtube") {
        // if there was no option, use the content of the [url] tag
        if(urlstart > 0 && m4l == 'youtube')
          return youtubeUrlConvert(string.substr(urlstart, offset-urlstart)) + "\">" + opentags.pop().etag;
        else if (urlstart > 0)
